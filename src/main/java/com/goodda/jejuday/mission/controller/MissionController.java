...
public class MissionController {
    ...
    @GetMapping
    public ResponseEntity<List<MissionThemeResponse>> getMissions() {
        return ResponseEntity.ok(missionQueryService.getMissionThemes());
    }

    @Operation(summary = "미션 테마 상세(스텝 목록) 조회", description = "특정 미션 테마에 속한 스텝(스팟) 목록과 각 스텝의 완료 여부를 조회합니다. 스탬프 카드 UI용.")
    @GetMapping("/{id}")
    public ResponseEntity<List<MissionStepResponse>> getMissionSteps(@PathVariable Long id) {
        return ResponseEntity.ok(missionQueryService.getMissionSteps(id));
    }
}
